...
class EarlyTrain(CoresetMethod):
    ...
    def run(self):
        ...
        try:
            ...
            self.model = nets.__dict__[self.args.model if self.specific_model is None else self.specific_model](
                num_classes = self.dst_pretrain_dict["num_classes"] if self.if_dst_pretrain else self.num_classes,
        ...
        # whole_model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32")
        # self.model.vision_model.load_state_dict(whole_model.vision_model.state_dict())
